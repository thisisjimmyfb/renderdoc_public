...
class AndroidRemoteServer(RemoteServer):
    ...
    def init(self, in_process):
        # Attempt to uninstall the demo and server, not checking for errors
        demo_package_name = util.get_android_demo_app_name()
        subprocess.run(['adb', '-s', self.device,
                        'uninstall', self.ADRD_SERVER_APP64], check=False)

        subprocess.run(['adb', '-s', self.device,
                        'uninstall', demo_package_name], check=False)
...
class LinuxRemoteServer(RemoteServer):
    TIMEOUT = 200
    PROCESS_NAME = 'renderdoccmd'

    def __ssh__(self, args):
        return f"ssh -p {self.port} {self.device} {util.get_shell()} -l -c \"\\\"{' '.join(args) if isinstance(args, list) else args}\\\"\""

    def __init__(self, device, port='22') -> None:
        super().__init__()
        if device is None:
            raise RuntimeError(
                'Linux remote target specified, but no device given')
        self.device = device
        self.port = port
        self.remote = None

        self.build_dir = util.get_renderdoccmd_dir()
        self._base_path ='/tmp/RenderDoc/'
        self._exe_path = self._base_path + LinuxRemoteServer.PROCESS_NAME
        self._demos_path = self._base_path + os.path.basename(util.get_demos_binary())

    def init(self, in_process):
        # make tmp dir
        subprocess.run(self.__ssh__(['mkdir', '-p', self._base_path + 'lib']), shell=True)

        # install renderdoccmd and dependencies for remote server
        log.print("Installing remote server...")
        subprocess.run(' '.join(['scp', '-P', self.port, self.build_dir + '/bin/' + LinuxRemoteServer.PROCESS_NAME,
                                 f"{self.device}:{self._base_path}"]), shell=True)
        log.print("Installing shared libraries...")
        subprocess.run(' '.join(['scp', '-P', self.port, self.build_dir + '/lib/librenderdoc.so',
                                 f"{self.device}:{self._base_path}/lib"]), shell=True)
        # install demos app
        log.print("Installing demos...")
        subprocess.run(' '.join(['scp', '-P', self.port, util.get_demos_binary(),
                                 f"{self.device}:{self._base_path}"]), shell=True)
        # Start remote server.  Don't use daemon mode, it doesn't play nicely on musl
        log.print("Starting remote server...")
        subprocess.run(self.__ssh__(f"export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{self._base_path}/lib; nohup {self._exe_path} remoteserver </dev/null >/dev/null 2>&1 &"),
                       timeout=LinuxRemoteServer.TIMEOUT, shell=True)
        sleep(5)
    ...
    def connect(self):
        ...
        result, remote = rd.CreateRemoteServerConnection(self.get_hostname())
        if result != rd.ResultCode.Succeeded:
            log.print(
    ...
    def shutdown(self):
        if self.is_connected():
            # self.remote.ShutdownServerAndConnection() causes a segfault here for some reason, so
            # just kill it the old fashioned way
            self.remote = None
            subprocess.run(self.__ssh__(['pkill', LinuxRemoteServer.PROCESS_NAME]),
                           timeout=LinuxRemoteServer.TIMEOUT, shell=True)

    def is_connected(self):
        return self.remote is not None

    def get_temp_path(self, name: str, timeout=TIMEOUT):
        subprocess.run(self.__ssh__(['mkdir', '-p', self._base_path + util.get_current_test()]),
                       timeout=timeout, check=True, shell=True)
        return self._base_path + util.get_current_test() + '/' + name

    def get_renderdoc_path(self):
        return self._base_path

    def path_exists(self, path, timeout=TIMEOUT):
        raw = subprocess.run(self.__ssh__(f'ls {path} >> /dev/null'),
                             stdout=subprocess.PIPE, timeout=timeout, shell=True).stdout
        raw = str(raw, 'utf-8').strip()
        return len(raw) == 0

    def run_demos(self, args: [str], timeout=TIMEOUT):
        # Run the command, blocking
        ret = subprocess.run(self.__ssh__(f"{self._demos_path} {' '.join(args)}"),
                             check=False, capture_output=True, timeout=timeout, shell=True)

        return str(ret.stdout, 'utf-8').strip()

    def inject_and_run_exe(self, cmdline, envmods, opts):
        package_and_activity = self._demos_path
        cmdline += " --max-frames 7"
        log.print("Running package:'{}' cmd:'{}' with env:'{}'".format(
            package_and_activity, cmdline, envmods))
        res = self.remote.ExecuteAndInject(
            package_and_activity, '.', cmdline, envmods, opts)
    ...
    def get_hostname(self):
        return re.split("@", self.device)[1]

    def get_username(self):
        return re.split("@", self.device)[0]

    def retrieve_latest_test_log(self, dst, timeout=10):
        # Remote Linux does not have per-test logging
        return None
    ...
    def retrieve_latest_server_log(self, dst, timeout=10):
        ...
        src = self.get_renderdoc_path()
        raw = subprocess.run(self.__ssh__(f"cd {src}; ls -t RenderDoc_* | head -1"),
                             check=False, stdout=subprocess.PIPE, timeout=timeout, shell=True).stdout
        latestlog = str(raw, 'utf-8').strip()
        if not latestlog:
            log.print(f"Cannot find latest remote log from '{src}'")
            return None
        ...
        src = os.path.join(src, latestlog)
        dst = os.path.join(dst, latestlog)
        log.print(f"Copying server remote log from '{src}' to '{dst}'")
        self.remote.CopyCaptureFromRemote(src, dst, None)
        log.print('Done')
    ...
    def retrieve_comms_log(self, timeout=10):
        ...
        src = self._base_path + "RemoteServer_Server.log"
        if not self.path_exists(src):
            log.print(f"Cannot find server comms log '{src}'")
            return None
